...
import MapSelector from '../../addProperty/MapSelector';
...
/* ── Reusable sub-components (same as upload form) ── */
function SectionHeader({ icon: Icon, title, optional }: { icon: React.FC<{ className?: string }>; title: string; optional?: boolean }) {
  return (
    <div className="flex items-center gap-2.5">
      ...
      {optional && <span className="text-[11px] font-semibold uppercase tracking-[0.06em] text-muted-foreground/60 ml-1">Optional</span>}
    </div>
  );
}
...
function StatusBadge({ status }: { status: string }) {
  ...
  return (
    <span className={cn('inline-flex items-center rounded px-2 py-0.5 text-[11px] font-semibold leading-none', map[status] ?? 'bg-muted text-muted-foreground border border-border')}>
      {status}
    </span>
  );
}

interface Props { warehouseId: string; initialData: any; }
...
export default function WarehouseEditForm({ warehouseId, initialData }: Props) {
  ...
  const allImageUrls     = [...existingImages.map(i => i.s3_url), ...newImagePreviews];
  ...
  const validateField = (name: string, value: any): string | undefined => {
    switch (name) {
      case 'title':
        ...
        break;
      case 'propertyType':  if (!value) return 'Property type is required'; break;
      case 'totalArea':
        if (!value) return 'Total area is required';
        if (parseFloat(value) <= 0) return 'Must be greater than 0';
        break;
      case 'availableFrom': if (!value) return 'Available from date is required'; break;
      case 'pricePerSqFt':
        if (!value) return 'Price per sq.ft is required';
        if (parseFloat(value) <= 0) return 'Must be greater than 0';
        break;
      case 'address':  if (!value?.trim()) return 'Address is required'; break;
      case 'city':     if (!value?.trim()) return 'City is required'; break;
      case 'state':    if (!value) return 'State is required'; break;
      case 'pincode':
  ...
  const handleNewImageChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (totalImageCount + files.length > MAX_IMAGES) {
      setFieldErrors(prev => ({ ...prev, images: `Max ${MAX_IMAGES} images. You can add ${MAX_IMAGES - totalImageCount} more.` }));
      return;
    }
  ...
  const handleNewVideoChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    const files = Array.from(e.target.files || []);
    if (!files.length) return;
    if (totalVideos + files.length > MAX_VIDEOS) { toast.error(`Max ${MAX_VIDEOS} videos allowed`); return; }
    if (files.some(f => !f.type.startsWith('video/'))) { toast.error('Only video files allowed'); return; }
    if (files.some(f => f.size > MAX_VIDEO_SIZE)) { toast.error('Some videos exceed 100MB limit'); return; }
  ...
  const removeExistingVideo = (index: number) => {
    setDeletedVideoIds(prev => [...prev, existingVideos[index].id]);
    ...
    <div className="min-h-screen bg-background py-8 px-6 lg:px-8">
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <div className="flex items-center gap-2 text-[13px] text-muted-foreground">
          ...
          <span>/</span>
          <Link href="/property" className="hover:text-foreground transition-colors">My Listings</Link>
          <span>/</span>
          <span className="text-foreground font-medium">Edit Property</span>
        </div>
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-[22px] font-semibold text-foreground tracking-tight">Edit Property</h1>
              <div className="flex items-center gap-2 mt-0.5">
                <StatusBadge status={initialData.status} />
                <span className="text-[12px] text-muted-foreground">
                  Listed {new Date(initialData.created_at).toLocaleDateString('en-IN', { day: 'numeric', month: 'short', year: 'numeric' })}
                </span>
              </div>
            </div>
          </div>
          ...
          <div className="lg:col-span-2 space-y-5">
            ...
            <Card className="border-border bg-card shadow-none">
              ...
              <CardContent className="px-5 pt-5 pb-5 space-y-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="space-y-1.5">
                    <Label htmlFor="propertyType" className="text-[12.5px] font-medium text-muted-foreground">Property Type<Req /></Label>
            ...
            <Card className="border-border bg-card shadow-none">
              ...
              <CardContent className="px-5 pt-5 pb-5 space-y-4">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="space-y-1.5">
                    <Label htmlFor="availableFrom" className="text-[12.5px] font-medium text-muted-foreground">Available From<Req /></Label>
                    <Input id="availableFrom" type="date" min={today} value={formData.availableFrom}
                  ...
                  <div className="space-y-1.5">
                    <Label htmlFor="listingType" className="text-[12.5px] font-medium text-muted-foreground">Listing Type<Req /></Label>
                    <Select value={formData.listingType} onValueChange={v => setFormData(p => ({ ...p, listingType: v as any }))}>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="space-y-1.5">
                    ...
                    <div className="relative">
                      ...
                      <Input id="pricePerSqFt" type="number" min="0" value={formData.pricePerSqFt}
            ...
            <Card className="border-border bg-card shadow-none">
              ...
              <CardContent className="px-5 pt-5 pb-5 space-y-4">
                <div className="space-y-1.5">
                  <Label htmlFor="address" className="text-[12.5px] font-medium text-muted-foreground">Address<Req /></Label>
                  <Textarea id="address" value={formData.address}
                    onChange={e => handleAddressChange(e.target.value)}
                    onBlur={() => handleFieldBlur('address')}
                    placeholder="Enter full street address" rows={2}
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="space-y-1.5">
                    ...
                    <Input id="pincode" maxLength={6} value={formData.pincode}
                  ...
                  <div className="space-y-1.5">
                    ...
                    <Select value={formData.roadConnectivity} onValueChange={v => setFormData(p => ({ ...p, roadConnectivity: v }))}>
                      ...
                      <SelectContent>{ROAD_CONNECTIVITY.map(r => <SelectItem key={r} value={r}>{r}</SelectItem>)}</SelectContent>
                    </Select>
                  </div>
                </div>
              ...
              <CardContent className="px-5 pt-5 pb-5">
                ...
                <div className="grid grid-cols-2 md:grid-cols-3 gap-2">
                  {AMENITIES.map(amenity => (
                    <label key={amenity} htmlFor={`edit-${amenity}`}
                      className={cn(
            ...
            <div className="sticky top-6 space-y-5">
                  ...
                  {allImageUrls.length === 0 ? (
                    <div>
                      <Label htmlFor="images" className="block cursor-pointer">
                        <div className={cn(
                          ...
                          touchedFields.has('images') && fieldErrors.images ? 'border-destructive' : 'border-border hover:border-foreground/30'
                        )}>
                          ...
                          <p className="text-[12px] text-muted-foreground">JPG, PNG, WEBP up to 5MB</p>
                        </div>
                      </Label>
                    ...
                    <div className="space-y-3">
                      <div className="grid grid-cols-3 gap-2">
                        {allImageUrls.slice(0, visibleCount).map((url, idx) => {
                          ...
                          return (
                            <div key={idx} className="relative group aspect-square">
                              ...
                              <div className="absolute inset-0 bg-black/0 group-hover:bg-black/15 rounded-md transition-colors flex items-center justify-center cursor-pointer" onClick={() => openGallery(idx)}>
                                <Eye className="h-4 w-4 text-white opacity-0 group-hover:opacity-100 transition-opacity" />
                              </div>
                              {!isExisting && (
                                <span className="absolute bottom-1 left-1 bg-foreground text-background text-[9px] px-1.5 py-0.5 rounded font-semibold">New</span>
                              )}
                            </div>
                          );
                        })}
                        ...
                        {remainingImages > 0 && (
                          <div className="aspect-square bg-accent rounded-md border border-border flex items-center justify-center cursor-pointer hover:border-foreground/30 transition-colors"
                      ...
                      <div className="flex items-center justify-between">
                        <span className="text-[12px] text-muted-foreground">{totalImageCount}/{MAX_IMAGES} images</span>
              ...
              <Card className="border-border bg-card shadow-none">
                <CardHeader className="pb-4 border-b border-border px-5 pt-5">
                  ...
                  <CardDescription className="text-[12px] mt-0.5">{totalVideos}/{MAX_VIDEOS} videos uploaded</CardDescription>
                </CardHeader>
                <CardContent className="px-5 pt-5 pb-5">
                  {totalVideos < MAX_VIDEOS && (
                    <Label htmlFor="videos" className="block cursor-pointer mb-3">
                      <div className="border-2 border-dashed rounded-lg p-6 text-center transition-colors hover:bg-accent/50 border-border hover:border-foreground/30">
                        ...
                        <p className="text-[12px] text-muted-foreground">Max 100MB each</p>
                      </div>
                    </Label>
                  )}
                  ...
                  <div className="space-y-2">
                    {existingVideos.map((vid, idx) => (
                      <div key={`ev-${vid.id}`} className="relative group">
                        <video src={vid.s3_url} className="w-full h-28 object-cover rounded-md border border-border" controls />
                        <button type="button" onClick={() => removeExistingVideo(idx)}
                          className="absolute -top-1.5 -right-1.5 h-5 w-5 rounded-full bg-foreground text-background flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity shadow">
                          <X className="h-3 w-3" />
                        </button>
                    ...
                    {newVideoPreviews.map((src, idx) => (
                      <div key={`nv-${idx}`} className="relative group">
                        ...
                        <span className="absolute top-2 left-2 bg-foreground text-background text-[9px] px-1.5 py-0.5 rounded font-semibold">New</span>
                        <span className="absolute bottom-2 left-2 bg-background/80 text-foreground text-[11px] px-1.5 py-0.5 rounded border border-border backdrop-blur-sm">
              ...
              <Card className="border-border bg-card shadow-none">
                ...
                <CardContent className="px-5 pt-4 pb-5">
                  {[
                    ...
                    ['Media',         `${totalImageCount} images, ${totalVideos} videos`],
              ...
              <div className="space-y-2">
                <Button onClick={handleSubmit} disabled={uploading} className="w-full h-10 text-[13.5px] font-semibold">
                  {uploading ? (
                    <><div className="animate-spin rounded-full h-4 w-4 border-b-2 border-background mr-2" />Saving…</>
                  ) : (
            ...
            <div className="flex items-center justify-between">
              ...
              <DialogDescription className="text-[12.5px] text-muted-foreground">
                ...
                {selectedImageIndex >= existingImages.length && (
                  <span className="ml-2 inline-flex items-center rounded px-1.5 py-0.5 text-[10px] font-semibold bg-foreground text-background">New</span>
                )}
              </DialogDescription>
            </div>
          </DialogHeader>
            ...
            <div className="group relative w-full bg-muted rounded-lg overflow-hidden flex items-center justify-center">
              <div className="w-full flex items-center justify-center" style={{ height: 'clamp(300px, 60vh, 700px)' }}>
                {allImageUrls[selectedImageIndex] && (
                  <img src={allImageUrls[selectedImageIndex]} alt={`Image ${selectedImageIndex + 1}`}
                ...
                {allImageUrls.map((url, idx) => (
                  <button key={idx} onClick={() => setSelectedImageIndex(idx)}
                    className={cn(
                      ...
                      idx === selectedImageIndex ? 'border-foreground scale-105' : 'border-border hover:border-foreground/40'
                    )}>
                    <img src={url} alt={`Thumb ${idx + 1}`} className="w-full h-full object-cover" />
                    {idx >= existingImages.length && (
                      <span className="absolute bottom-0.5 right-0.5 bg-foreground text-background text-[8px] px-1 py-0.5 rounded font-bold">NEW</span>
                    )}
                  </button>
                ))}
              </div>
      ...
      <AlertDialog open={imageToDelete !== null} onOpenChange={() => setImageToDelete(null)}>
        <AlertDialogContent>
          ...
          <AlertDialogFooter>
            ...
            <AlertDialogAction className="h-9 text-[13px]"
              onClick={() => {
                if (!imageToDelete) return;
                imageToDelete.type === 'existing' ? removeExistingImage(imageToDelete.index) : removeNewImage(imageToDelete.index);
              }}>
              Remove
            </AlertDialogAction>
          </AlertDialogFooter>
